    ...
    if (subscriptionHasFreeTrial({sub: subscription})) {
        ...
        return (
            <p className="gh-portal-account-discountcontainer">
                <div>
                    <span>Gratis prova på – Slutar om {trialEnd}</span>
                    {/* <span>{getSubFreeTrialDaysLeft({sub: subscription})} dagar</span> */}
                </div>
            </p>
        );
    }
...
const PaidAccountActions = () => {
    ...
    const PlanLabel = ({price, isComplimentary, subscription}) => {
        ...
        if (isComplimentary) {
            if (compExpiry) {
                label = `Kostnadsfri - Avslutas ${compExpiry}`;
            } else {
        ...
        <div>
            <div className='gh-portal-list'>
                <section>
                    ...
                    <button className='gh-portal-btn gh-portal-btn-list' onClick={e => openEditProfile(e)}>Ändra</button>
                </section>
...
function EmailPreferencesAction() {
    ...
    return (
        <section>
            <div className='gh-portal-list-detail'>
                <h3>E-postmeddelanden</h3>
                <p>Uppdatera dina inställningar</p>
            </div>
...
const AccountWelcome = () => {
    ...
    if (subscription) {
        ...
        if (isComplimentary && getCompExpiry({member})) {
            ...
            return (
                <div className='gh-portal-section'>
                    <p className='gh-portal-text-center gh-portal-free-ctatext'>Ditt abonnemang löper ut den {expiryAt}</p>
                </div>
            );
        }
        ...
        if (subscriptionHasFreeTrial({sub: subscription})) {
            ...
            return (
                <div className='gh-portal-section'>
                    <p className='gh-portal-text-center gh-portal-free-ctatext'>Ditt abonnemang börjar den {trialEnd}</p>
                </div>
            );
        }
        return (
