...
const Header = ({onBack, showConfirmation, confirmationType}) => {
    ...
    let title = isPaidMember({member}) ? 'Ändra nivå' : 'Välj en nivå';
    if (showConfirmation) {
    ...
    if (type === 'changePlan') {
        ...
        return (
            <div className="gh-portal-logged-out-form-container gh-portal-cancellation-form">
                <p>Om du säger upp din prenumeration nu kommer du fortsätta ha tillgång tills <strong>{getDateString(subscription.current_period_end)}</strong>.</p>
