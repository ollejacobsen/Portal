...
function ProductCardTrialDays({trialDays, discount, selectedInterval}) {
    ...
    if (hasFreeTrialTier({site})) {
        if (trialDays) {
            return (
                <span className="gh-portal-discount-label">{trialDays} prova på dagar</span>
            );
        } else {
            return null;
        }
    ...
    if (selectedInterval === 'year') {
        return (
            <span className="gh-portal-discount-label">{discount}% rabatt</span>
        );
    }
                    ...
                    {(!hasOnlyFree ?
                        <div className='gh-portal-btn-product'>
                            ...
                            <button
                                ...
                                {((selectedProduct === 'free' && disabled) ? <LoaderIcon className='gh-portal-loadingicon' /> : 'Välj')}
                            </button>
                        </div>
                        : '')}
                </div>
...
function ProductCardButton({selectedProduct, product, disabled, noOfProducts, trialDays}) {
    ...
    if (trialDays > 0) {
        return ('Påbörja ' + trialDays + '-prova på dagar');
    }

    return (noOfProducts > 1 ? 'Välj' : 'fortsätt');
}
...
function ProductCard({product, products, selectedInterval, handleChooseSignup}) {
    ...
    if ((!product.benefits || !product.benefits.length) && !productDescription) {
        productDescription = 'Full åtkomst';
    }
...
function YearlyDiscount({discount, trialDays}) {
    ...
    if (hasFreeTrialTier({site})) {
        return (
            <>
                <span className="gh-portal-discount-label-trial">{discount}% rabatt</span>
            </>
        );
    } else {
        return (
    ...
    return (
        <div className={cardClass + (currentPlan ? ' disabled' : '')} key={product.id} onClick={(e) => {
            ...
            <div className='gh-portal-product-card-details'>
                ...
                {(currentPlan ?
                    <div className='gh-portal-btn-product'>
                        <span className='gh-portal-current-plan'><span>Nuvarande nivå</span></span>
                    </div>
                    :
                    <div className='gh-portal-btn-product'>
                        <button
